...
TEST(LoggerDistributor, LoggerDistributor) {
	...
	EXPECT_CALL(log1, Log("Log message", "op2ext.dll")).Times(1);
	EXPECT_CALL(log2, Log("Log message", "op2ext.dll")).Times(1);
	EXPECT_CALL(log3, Log("Log message", "op2ext.dll")).Times(1);
